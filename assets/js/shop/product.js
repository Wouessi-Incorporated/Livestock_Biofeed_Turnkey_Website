(async () => {
    ...
    agri.forEach(s => {
        ...
        c.className = "ecommerce-card";
        c.style.marginBottom = "10px";
        c.innerHTML = `
      <div class="ecommerce-p">
        <div class="ecommerce-h2" style="font-size:16px">${s.name}</div>
        <div class="ecommerce-muted" style="font-size:12px">${s.cta === "buy" ? "Pilot & Training Unit" : "Request a Quote"}</div>
        <div class="ecommerce-line"></div>
        <div class="ecommerce-btnrow">
          <a class="ecommerce-cta ${s.cta === 'buy' ? '' : 'secondary'}" style="font-size:12px; padding:8px 12px" href="product.html?id=${encodeURIComponent(s.id)}">${s.cta === 'buy' ? 'Buy Now' : 'Request a Quote'}</a>
        </div>
      </div>`;
        box.appendChild(c);
    });
    ...
    document.getElementById("buyNow").addEventListener("click", () => {
        if (!p.price) { location.href = "checkout.html"; return; }
        window.LBB_CART.add(p.id, 1);
